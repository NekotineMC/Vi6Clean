...
public class Vi6VoiceChatPlugin implements VoicechatPlugin {
	...
	private void onMic(MicrophonePacketEvent evt) {
		...
		for (int i = 0; i < soundData.length; i += severity) {
			// 0-100
			int sum = 0;
		...
		for (var p : game.getGuards()) {
			...
			var audioPlayer = audioPlayers
					.computeIfAbsent(p,
							pl -> api.createAudioPlayer(
									api.createStaticAudioChannel(p.getUniqueId(),
											connection.getPlayer().getServerLevel(), connection),
									api.createEncoder(), () -> {
			...
			// packet.staticSoundPacketBuilder().build());
		}
	}
