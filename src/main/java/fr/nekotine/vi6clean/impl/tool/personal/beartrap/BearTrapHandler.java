...
public class BearTrapHandler extends ToolHandler<BearTrap>{
	...
	private static final StatusEffect TRAPPED_EFFECT = new StatusEffect(
	...
	.postApply(i -> i.setData(DataComponentTypes.PROFILE, ResolvableProfile.resolvableProfile(ARMED_PLAYER_PROFILE)))
	.build();
	...
	private final ItemStack UNARMED_HELMET_ITEM = new ItemStackBuilder(Material.PLAYER_HEAD)
			...
			.unstackable()
			.flags(ItemFlag.values())
			.postApply(i -> i.setData(DataComponentTypes.PROFILE, ResolvableProfile.resolvableProfile(UNARMED_PLAYER_PROFILE)))
			.build();
	...
	private void onPlayerMove(PlayerMoveEvent evt) {
		...
		for(BearTrap tool : getTools()) {
			...
			var player = evt.getPlayer();
			if(wrappingModule.getWrapper(tool.getOwner(), PlayerWrapper.class).ennemiTeamInMap().noneMatch(player::equals)) {
				continue;
			}
			if(tool.getLocation().distanceSquared(evt.getFrom()) <= SQUARED_TRIGGER_RANGE) {
				tool.setArmed(false);
				tool.getLocation().getWorld().spawnEntity(
						player.getLocation(), EntityType.EVOKER_FANGS, SpawnReason.TRAP, fang -> {
