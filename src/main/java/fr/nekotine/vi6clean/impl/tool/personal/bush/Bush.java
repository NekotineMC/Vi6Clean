...
public class Bush extends Tool{
	...
	protected ItemStack makeInitialItemStack() {
		return ItemStackUtil.make(Material.SILVERFISH_SPAWN_EGG,BushHandler.NB_MAX_WATCHER,
				Component.text("Observateur",NamedTextColor.GOLD),
				Vi6ToolLoreText.WATCHER.make());
	}
	
	public void lowTick() {
		if (watchers.size() < BushHandler.NB_MAX_WATCHER) {
			var player = getOwner();
			...
			var y = loc.getY();
			var z = loc.getZ();
			SpatialUtil.circle2DDensity(BushHandler.DETECTION_BLOCK_RANGE, 5, 0,
					(offsetX, offsetZ) -> {
	...
	public boolean tryPickupWatcher() {
		...
		var ownerLoc = owner.getLocation();
		var worked = false;
		for (var sf : watchers.stream().filter(sf -> sf.getLocation().distanceSquared(ownerLoc) <= BushHandler.DETECTION_RANGE_SQUARED)
	...
	public void itemUpdate() {
		var nbAvailable = BushHandler.NB_MAX_WATCHER-watchers.size();
		if (nbAvailable <= 0) {
