...
public class Radar extends Tool {
	...
	protected void detect() {
		if (placed) {
			...
			var ennemiNear = opt.get().ennemiTeamInMap().filter(
					e -> bottom.getLocation().distanceSquared(e.getLocation()) <= handler.getDetectionRangeSquared())
			...
			RadarHandler.BALL.forEach(triplet -> {
	...
	protected void tickParticle() {
		if (placed) {
			...
			var z = loc.getZ();
			RadarHandler.BALL.forEach(triplet -> {
				loc.getWorld().spawnParticle(Particle.WITCH, x + triplet.a(), y + triplet.b(), z + triplet.c(), 1, 0, 0,
						0, 0, null);
			});
			...
			var y = loc.getY();
			var z = loc.getZ();
			RadarHandler.SPHERE.forEach(triplet -> {
				getOwner().spawnParticle(Particle.WITCH, x + triplet.a(), y + triplet.b(), z + triplet.c(), 1, 0, 0, 0,
						0, null);
			});
		}
	}
