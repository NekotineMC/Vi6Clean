...
public class WarnerHandler extends ToolHandler<WarnerHandler.Warner>{
	...
	private void onPlayerInterract(PlayerInteractEvent evt) {
		...
		var nearestAvailableArtefact = map.getArtefacts().values().stream().filter(art -> 
			 (!art.isCaptured()) && 
			 (art.getBlockPosition().toLocation(world).distanceSquared(player.getLocation()) <= PLACE_RANGE_SQUARED) && !watchedArtefacts.contains(art.getName())
		).findFirst();
		...
		tool.eye_item1 = (ItemDisplay)world.spawnEntity(watchedLocation.clone().add(0.5, 0.5, 0.5), EntityType.ITEM_DISPLAY, SpawnReason.CUSTOM, e -> {
		...
		tool.eye_item2 = (ItemDisplay)world.spawnEntity(watchedLocation.clone().add(0.5, 0.5, 0.5), EntityType.ITEM_DISPLAY, SpawnReason.CUSTOM, e -> {
			if (e instanceof ItemDisplay dis) {
				dis.setItemStack(new ItemStack(Material.ENDER_EYE));
		...
		while(iterator.hasNext()) {
			...
			if(tool.watched != null && tool.watched.isCaptured() && ++tool.warnDelay >= WARN_DELAY_TICK) {
				var message = Ioc.resolve(TextModule.class).message(Leaf.builder()
						.addStyle(Placeholder.unparsed("name", tool.watched.getName()))
			...
			if(evt.timeStampReached(TickTimeStamp.HalfSecond)) {
				...
				if(tool.watched != null) {
					...
					var y = loc.getY();
					var z = loc.getZ();
					SpatialUtil.circle2DDensity(PLACE_RANGE, 5, 0,(offsetX, offsetZ) -> 
