...
public class DephaserHandler extends ToolHandler<Dephaser>{
	...
	private final int DELAY_BETWEEN_INVISIBILITY_TICKS = Tick.tick().fromDuration(TimeUtil.fromSeconds(getConfiguration().getDouble("inv_delay", 20)));
	
	private final int INVISIBILITY_DURATION_TICKS = Tick.tick().fromDuration(TimeUtil.fromSeconds(getConfiguration().getDouble("inv_duration", 2)));
	...
	public void onTick(TickElapsedEvent evt) {
		...
		if (count == DELAY_BETWEEN_INVISIBILITY_TICKS-INVISIBILITY_DURATION_TICKS+DELAY_BETWEEN_WARNING_SOUND*2) {
			getTools().stream().filter(t -> t.isActive()).forEach(t -> Vi6Sound.DEPHASER_WARNING_HIGH.play(t.getOwner()));
			return;
		}
		if (count == DELAY_BETWEEN_INVISIBILITY_TICKS-INVISIBILITY_DURATION_TICKS+DELAY_BETWEEN_WARNING_SOUND) {
			getTools().stream().filter(t -> t.isActive()).forEach(t -> Vi6Sound.DEPHASER_WARNING_MID.play(t.getOwner()));
			return;
		}
	...
	private void onEmpStart(EntityEmpStartEvent evt) {
		if (evt.getEntity() instanceof Player p) {
			InventoryUtil.taggedItems(p.getInventory(), TOOL_TYPE_KEY, getToolCode()).stream().map(this::getToolFromItem).filter(t -> t != null && t.getOwner() != null).forEach(this::deactivate);
		}
	}
}
